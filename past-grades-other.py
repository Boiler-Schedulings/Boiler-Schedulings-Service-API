...
class ExcelDataProcessor:
    ...
    def get_info_by_subject_and_course(self, course_number, subject):
        ...
        #print(grade_row_condition)
        course_subject_rows = (self.excel_data['Course Number'] == course_number) & (
                    self.excel_data['Subject'].str.strip().str.lower() == subject.lower())
        selected_info = self.excel_data.loc[(course_subject_rows) & grade_row_condition, grade_columns].values
...
course_number = 20300
subject = 'AAE'
info = data_processor.get_info_by_subject_and_course(course_number, subject)

if info is not None:
    print(f"Information for Course {course_number} and Subject {subject}:")
    print(info)
